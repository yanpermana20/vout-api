...
function throwQuestion($userId, $question, $timeLimit, $location, $options, $targets, $category_id){
	...
	if($numOption >= 2){
		...
		$query_get_user_info="select first_name from users where id='".$userId."'";
		$result_get_user_info=mysql_query($query_get_user_info);
		$user_info=mysql_fetch_assoc($result_get_user_info);
		...
		if($resultQuestion){
			...
			if($resultActivity){
				if(!empty($targets)){
					...
					$arrPushMessage['message']=$user_info['first_name']." tagged you on a question.";
					$pushMessage=json_encode($arrPushMessage);
					
					if(!empty($arrDeviceFriendAndroid)){
						$pushAndroidResult=android_push_notif($arrDeviceFriendAndroid,"TAGGED",$pushMessage);
						$arrPushAndroidResult=json_decode($pushAndroidResult);
					}
					if(!empty($arrDeviceFriendIos)){
						$pushIosResult=ios_push_notif($arrDeviceFriendIos,"TAGGED",$pushRequestMessage);
					}
				}
