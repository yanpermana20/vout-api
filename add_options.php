...
if(!empty($headers['HTTP_TOKEN']) && !empty($headers['HTTP_UUID'])){
	...
	if($user_info->status == 1){
		if(!empty($_POST['title']) && !empty($_POST['image_url'])){
			...
			$addOptions=add_options($user_info->user_id,$title,$description,$image_id,$image_url,$image_source,$category_id);
			echo $addOptions;
		}else{
			$data['status']="0";
...
function add_options($userId, $title, $description, $image_id, $image_url, $image_source, $category_id){
	...
	if(!empty($imageId)){
		...
		$data['status']="0";
		$data['message']="Option Image empty ".$image_url;
	}
