...
if(!empty($headers['HTTP_TOKEN']) && !empty($headers['HTTP_UUID'])){
	...
	if($user_info->status == 1){
		if(!empty($_REQUEST['friend_id'])){
			...
			$approve_friend=approve_friend($user_info->user_id,$friend_id,$device);
			echo $approve_friend;
		}else{
			$data['status']="0";
			$data['message']="Friend id or device type is empty";
...
// $approve_friend=approve_friend('5','1','android');
