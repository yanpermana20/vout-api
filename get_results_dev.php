...
$result=get_resutls('882398e63033d68b36cc019ac62c0eec');
echo "<pre>";
print_r($result);
function get_resutls($user_id,$totalRow,$page){
	...
	if($result_get_activities){
		...
		if(!empty($questionId)){
			$query_get_questions="select q.id, q.user_id, q.question, q.hit_rate, q.vout_count, q.time_limit, q.options theOptions, q.location, q.is_private, q.target_id, 
