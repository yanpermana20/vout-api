...
// if(!empty($headers['HTTP_TOKEN']) && !empty($headers['HTTP_UUID'])){
	...
	// if($user_info->status == 1){
		...
		// $page=$_POST['page'];
		// $peopleQuestions=getPeopleQuestions($user_info->user_id,$people_id,$facebook_id,$totalRow,$page);
		// //echo "<pre>";
		// echo $peopleQuestions;
	// }else{
		// echo $token_cek;
	// }
// }else{
...
$myQuestions=getPeopleQuestions('623b55ae38d2ce82c1739fa6d021f3d9','623b55ae38d2ce82c1739fa6d021f3d9','',5,0);
echo "<pre>";
print_r($myQuestions);

function getPeopleQuestions($userId, $peopleId, $facebookId, $totalRow, $page){
	if(empty($totalRow)){
		$rowsPerPage=10;
	}else{
	...
	$query2="select q.id, q.user_id, q.question, q.comments, q.category_id, q.hit_rate, q.vout_count, q.time_limit, q.options, q.location, q.is_private, q.target_id, 
			...
			where u.".$string_id."='".$id."' and q.status='1' and time_limit > now() order by q.created_date desc limit ".$offset.", ".$rowsPerPage;
